...
// Cloudflare Worker: Complete License API & Web Admin Dashboard for Submarine
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/api/verify") {
      ...
      if (!match) {
        return new Response(JSON.stringify({ valid: false, error: "Kunci akses tidak terdaftar" }), {
          status: 404,
          headers: corsHeaders,
        });
      }
...
<body>
  <div class="container">
    <header>
      <div class="brand">
        ...
        <div>
          ...
          <p>Sistem Pengendalian Lisensi & Kriptografi Serial Key</p>
        </div>
      </div>
    ...
    <div id="dashboardSection" style="display:none;">
      <div class="stats-grid">
        ...
        <div class="stat-card">
          <h3>Tipe Enkripsi</h3>
          <div class="val" style="color:#22d3ee; font-size:20px;">HMAC SHA-256</div>
        </div>
      </div>
      ...
      <div class="card">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px; flex-wrap:wrap; gap:12px;">
          <div>
            <h2 style="font-size:18px; font-weight:700;">Daftar Kunci Akses Resmi Teman</h2>
            <p style="color:var(--text-dim); font-size:13px;">Setiap kunci memiliki checksum kriptografi unik. Salin dan kirimkan ke temanmu.</p>
          </div>
  ...
  <div class="modal" id="createModal">
    <div class="modal-content">
      <div class="modal-header">
        <h3 style="font-size:18px;">+ Generate Kunci Serial Baru</h3>
      ...
      <div style="background:rgba(99,102,241,0.1); border:1px solid rgba(99,102,241,0.25); border-radius:10px; padding:12px; font-size:12px; color:#a5b4fc; line-height:1.4;">
        💡 Sistem akan secara otomatis menghitung checksum kriptografi SHA-256 untuk nama teman tersebut agar 100% aman dan tidak bisa ditebak orang lain.
      </div>
      <div style="display:flex; justify-content:flex-end; gap:10px; margin-top:24px;">
        <button class="btn btn-outline" onclick="closeCreateModal()">Batal</button>
        <button class="btn btn-primary" onclick="submitCreateKey()">Generate Kunci 🔑</button>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    var SALT = "SubMarine_Secure_2026_Salt";
    ...
    async function calculateChecksum(name) {
      var cleanName = name.trim().toUpperCase().replace(/[^A-Z0-9]/g, "");
      var msgBuffer = new TextEncoder().encode(cleanName + ":" + SALT);
      var hashBuffer = await crypto.subtle.digest("SHA-256", msgBuffer);
      var hashArray = Array.from(new Uint8Array(hashBuffer));
      var hashHex = hashArray.map(function(b) { return b.toString(16).padStart(2, "0"); }).join("").toUpperCase().substring(0, 6);
      return "SUB-" + cleanName + "-" + hashHex;
    }
    ...
    async function submitCreateKey() {
      ...
      var code = await calculateChecksum(rawName);
